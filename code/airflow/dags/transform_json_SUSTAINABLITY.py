...
with DAG(
    "Transform_JSON_Data_Sustainability",
    ...
    list_files_in_sustainability_folder = GCSListObjectsOperator(
        task_id="list_files_in_sustainability_folder",
        bucket=RAW_BUCKET,
        prefix="raw/api/sustainability/",
        delimiter="/",
    )
    ...
    def process_files(**kwargs):
        ti = kwargs["ti"]
        files = ti.xcom_pull(task_ids="list_files_in_sustainability_folder")
        ...
        for file_object in files:
            ...
            dataproc_job_config = {
                ...
                "pyspark_job": {
                    "main_python_file_uri": f"gs://{RAW_BUCKET}/scripts/transform_and_copy_to_landing_sustainability.py",
                    "args": [
                        "--input-bucket",
                        RAW_BUCKET,
                        "--input-file",
                        f"raw/api/sustainability/{file_name}",
                        "--output-bucket",
                        LANDING_BUCKET,
                        "--output-file",
                        f"sustainability/{file_name}",
                    ],
                },
            }
    ...
    load_to_bigquery = BigQueryInsertJobOperator(
        ...
        configuration={
            "load": {
                "sourceUris": [f"gs://{LANDING_BUCKET}/sustainability/*"],
                "destinationTable": {
                    "projectId": PROJECT_ID,
                    "datasetId": BQ_DATASET,
                    "tableId": "sustainability",
                },
    ...
    list_files_in_sustainability_folder >> process_files_task >> load_to_bigquery
