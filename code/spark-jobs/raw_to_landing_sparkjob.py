...
from pyspark.sql.functions import col, lit, regexp_replace, row_number, to_date, year
...
def standardize_kaggle_data(spark, local_path):
    ...
    kaggle_schema = StructType(
        [
            ...
            StructField(
                "Open", StringType(), True
            ),  # Read as string to handle any formatting issues
            StructField("High", StringType(), True),
            StructField("Low", StringType(), True),
            StructField("Close", StringType(), True),
            StructField("Volume", StringType(), True),  # Volume as string due to commas
            StructField(
                "OpenInt", StringType(), True
            ),  # Assuming OpenInt may be unnecessary
        ]
    )

    # Read the Kaggle data with the defined schema
    kaggle_df = spark.read.csv(local_path, schema=kaggle_schema, header=True)

    # Convert columns to proper data types
    kaggle_df = kaggle_df.withColumn("Date", to_date(col("Date"), "yyyy-MM-dd")).filter(
        col("Date").isNotNull()
    )  # Remove rows with invalid dates

    # Cast numeric columns
    kaggle_df = (
        kaggle_df.withColumn("Open", col("Open").cast("double"))
        .withColumn("High", col("High").cast("double"))
        .withColumn("Low", col("Low").cast("double"))
        .withColumn("Close", col("Close").cast("double"))
        .withColumn(
            "Volume",
            regexp_replace(col("Volume"), ",", "").cast("double"),  # Remove commas
        )
        .drop("OpenInt")
...
def move_raw_to_staging(spark, kaggle_path, api_path, TICKER):
    ...
    final_df = combined_df.withColumn("year", year(combined_df.Date)).withColumn(
        "Ticker", lit(f"{TICKER}")
    )
...
# Main logic
spark = SparkSession.builder.appName("Fortune500").getOrCreate()
...
if final_df_list:
    ...
    combined_final_df.write.partitionBy("year").mode("overwrite").parquet(
        f"gs://{GCP_LANDING_BUCKET}/stock_data/"
    )
